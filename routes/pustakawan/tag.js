...
router.get('/', authPustakawan, async (req, res) => {
    try {
        ...
        if (flashedKeyword) {
            const data = await Tag.searchByNama(flashedKeyword)
            return res.render('pustakawan/tag/index', {
                data,
                pegawai,
                page: 1,
                totalHalaman: 1,
        ...
        const data = await Tag.getTag(limit, offset)
        const totalData = await Tag.countTag()
        const totalHalaman = Math.ceil(totalData / limit)

        res.render('pustakawan/tag/index', {
            data,
            pegawai,
            page,
            totalHalaman
...
router.post('/delete/:id', authPustakawan, async (req, res) => {
    try {
        ...
        if (await Tag.checkTagUsed(id)) {
            req.flash('error', 'Tag masih digunakan pada blog')
            return res.redirect('/pustakawan/tag')
        }
